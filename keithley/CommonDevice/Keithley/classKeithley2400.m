...
% Last Modified on 2018-03-23
    ...
    methods        
        ...
        function err = setSweepV(device,start,stop,step,delay,varargin)
            ...
            addRequired(input,'delay',validationFcn('keithleyDelay',device.functionName));
            addOptional(input,'integrationRate',device.intRate,validationFcn('keithleyIntegrationRate',device.functionName));
			addOptional(input,'complianceLevel',device.compLvl,validationFcn('keithleyComplianceLevel',device.functionName));
            ...
            err = device.updateSweepV(input.Results.start,input.Results.stop,input.Results.step,input.Results.delay,input.Results.integrationRate,input.Results.complianceLevel,'spacing',input.Results.spacing);
            if err
                return
            end
        ...
        function err = updateSweepV(device,start,stop,step,delay,varargin)
            ...
            %# set sense function (device, mode [I,V,R], int_rate, prot_lvl, range_auto, range)
            err = setSense_Keithley(device.connectionHandle, 'I', 'integrationRate',input.Results.integrationRate,'complianceLevel',input.Results.complianceLevel);
            if err
                return
            end
        end
